...
abstract class InstallationProcess(private val name: String)
{
    ...
    //<editor-fold desc="Rendering Fields">
    /**
     * Icon to display when the process has finished.
     */
    private val finishIcon = ImageIO.read(javaClass.getResourceAsStream("/status/finish.png"))

    /**
     * Icon to display when an error occurred.
     */
    private val errorIcon = ImageIO.read(javaClass.getResourceAsStream("/status/error.png"))
    ...
    fun render(graphics2D: Graphics2D, x: Int, y: Int)
    {
        ...
        val arc1 = Arc2D.Double(x.toDouble() - flyIn.get() * 20, (y - 20).toDouble(), 24.0, 24.0, 15.0 - close + loading, 180.0 - close * 3, Arc2D.PIE)
        val arc2 = Arc2D.Double(x.toDouble() - flyIn.get() * 20, (y - 20).toDouble(), 24.0, 24.0, 195.0 - close + loading, 180.0 - close * 3, Arc2D.PIE)
        ...
        graphics2D.drawImage(if (status == -1) errorIcon else finishIcon, centerX - size / 2, centerY - size / 2, size, size, null)
